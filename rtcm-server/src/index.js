...
var count = 0;
// var port = 2000;
const PORT = process.env.RTCM_SERVER_PORT;
...
rtcm.on("connection", function (socket) {
  ...
  socket.on("room", function (room) {
    ...
    console.log(
      "JOINED TO: " + room + " users in room: " + usersInRomm.get(room)
    );
  ...
  socket.on("featureInfoTool", function (data) {
    ...
    socket.broadcast
      .to(data.room)
      .emit("featureInfoTool", data.featureId, data.column, data.value);
  });
...
http.listen(PORT, function () {
  console.log("RTCMapping Server listening on *:" + PORT);
});
