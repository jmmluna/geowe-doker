...
var RealTimeEngine = function (options) {
  ...
  this.counterMessageDefault = document.getElementById(
    "counter-" + this.device
  ).innerText;
};
...
RealTimeEngine.prototype.connect = function () {
  ...
  this.socket = io(RTCM_SERVER_URL);
  this.listen();
};
 ...
 */
RealTimeEngine.prototype.emitAttributeNewValue = function (
  featureId,
  attributeName,
  attributeValue
) {
...
RealTimeEngine.prototype.delete = function (featureId) {
  this.socket.emit(RteOperation.DELETE, {
    featureId: featureId,
    room: this.appContext.getRoom(),
  });
};
...
RealTimeEngine.prototype.listen = function () {
  ...
  this.socket.on(RteOperation.DISCONNECT, function () {
    ...
    document.getElementById("counter-" + this_.device).innerHTML =
      this_.counterMessageDefault;
  ...
  this.socket.on(RteOperation.COUNTER, function (data) {
    document.getElementById("counter-" + this_.device).innerHTML =
      '<i class="icon-group"></i> ' + data.count + " usuarios conectados ";
    colaborators.innerHTML =
      '<i class="icon-group"></i> Conectados: ' + data.count;
  ...
  this.socket.on(RteOperation.NICK, function (data) {
    if (this_.nick === "no-nick") {
      var newNick = prompt("¿Quieres cambiar tu nick?", data.nick);
      if (newNick != null) {
        this_.nick = newNick;
      } else {
    ...
    if (button.id == "shareTool-desktop") {
      button.innerHTML =
        '<i class="icon-check-circle"></i> Desconectar <font size="2">(' +
        this_.nick +
        ")</font>";
    } else {
      button.innerHTML = '<i class="icon-check-circle"></i>';
      status.innerHTML =
        '<i class="icon-check-circle"></i> Conectado: <font size="2">(' +
        this_.nick +
        ")</font>";
    }
  ...
  this.socket.on(RteOperation.MODIFY, function (geojson) {
    ...
    source.addFeature(
      buildFeature(new FeatureMapper(appContext).toGeojson(featureToModify))
    );

    new Notificator().notify(
      "Se ha Modificado el elemento: " + receivedFeature.getId()
    );
  });
  ...
  this.socket.on(RteOperation.INFO_TOOL, function (featureId, column, value) {
    ...
    featureInfoTool.updateGrid();
    new Notificator().notify(
      "Se ha modificado alfanuméricamente el elemento: " + featureId
    );
  });
  ...
  this.socket.on(RteOperation.NEW_ATTRIBUTE, function (newColumn) {
    ...
    featureInfoTool.updateGrid();
    new Notificator().notify(
      "Se ha añadido un nuevo atributo a la capa de trabajo: " + newColumn
    );
  });
};
